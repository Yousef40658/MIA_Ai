...
class DataExplorer() :
    ...
    def __init__(self, file_path):
        ...
        try:
            if file_path.endswith('.csv'):
                try:
                    self.df = pd.read_csv(file_path, low_memory=False, encoding='utf-8')
                except UnicodeDecodeError:
                    self.df = pd.read_csv(file_path, low_memory=False, encoding='cp1252')
            ...
            raise  # <-- don't let __init__ silently continue with no self.df
    ...
    def get_correlations(self, top_n=8):
