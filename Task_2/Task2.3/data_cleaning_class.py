...
class DataCleaner():
    ...
    def handle_outliers(self, z_threshold=4):
        cols_to_check = self.numerical_df.columns.drop("value", errors="ignore")
        z = np.abs(zscore(self.numerical_df[cols_to_check], nan_policy="omit"))
        z_df = pd.DataFrame(z, columns=cols_to_check, index=self.numerical_df.index)
