...
class DataCleaner():
    ...
    def handle_missing(self, col_threshold=0.3, row_threshold=0.3):
        ...
        #fill any remaining numeric nulls with the column mean
        self.numerical_df = self.df.select_dtypes(include='number')
    ...
    # since data is highly skewed there is some outliers but some of them are realistic like high n of goals should be given high rating
    def handle_outliers(self, z_threshold=3, cols=None):
            # if cols is None:
            #     cols = ['age', 'height_cm', 'weight_kg', 'market_value_eur']
            # cols = [c for c in cols if c in self.df.columns]
            cols = self.numerical_df.columns
            z = np.abs(zscore(self.df[cols], nan_policy="omit"))
            z_df = pd.DataFrame(z, columns=cols, index=self.df.index)
